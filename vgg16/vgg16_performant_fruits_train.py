...
trainloader = torch.utils.data.DataLoader(train_data, batch_size=49, shuffle=True, num_workers=2)
...
testloader = torch.utils.data.DataLoader(test_data, batch_size=49, shuffle=False, num_workers=2)
...
class VGG(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(719 * 3 * 3, 4067),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(4067, 8916),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(8916, num_classes),
        )
        if init_weights:
...
cfgs: Dict[str, List[Union[str, int]]] = {
    'VGG16': [192, 192, 'M', 189, 189, 'M', 383, 383, 383, 'M', 741, 741, 741, 'M', 719, 719, 719, 'M'],
}
