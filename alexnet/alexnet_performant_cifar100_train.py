...
trainloader = torch.utils.data.DataLoader(train_data, batch_size=123, shuffle=True, num_workers=2)

#Downloading test data
test_data = torchvision.datasets.CIFAR100(root='./data', train=False, download=True, transform=transform)
#test_data = torchvision.datasets.FashionMNIST(root='./data', train=False, download=True, transform=transform)

testloader = torch.utils.data.DataLoader(test_data, batch_size=123, shuffle=False, num_workers=2)
...
class AlexNet(nn.Module):
    ...
    def __init__(self, num_classes: int = 100) -> None:
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 98, kernel_size=10, stride=4, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(98, 294, kernel_size=8, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=5, stride=2),
            nn.Conv2d(294, 416, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(416, 432, kernel_size=8, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(432, 251, kernel_size=4, padding=1),
            nn.ReLU(inplace=True),
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(),
            nn.Linear(251 * 2 * 2, 1249),
            nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(1249, 3488),
            nn.ReLU(inplace=True),
            nn.Linear(3488, num_classes),
        )
