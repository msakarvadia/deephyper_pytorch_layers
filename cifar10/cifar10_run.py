   ...
   try:
      ...
      class Net(torch.nn.Module):
         def __init__(self, batch_size,
            ...
            self.flop = 0
            self.conv1 = torch.nn.Conv2d(conv1_in_chan, conv1_out_chan, conv1_kern).to(device, dtype=torch.float32)
            self.flop += conv1_kern**2 * conv1_in_chan * conv1_out_chan * image_size**2 * batch_size
            print(self.flop)
            self.pool  = torch.nn.MaxPool2d(pool_size, pool_size).to(device, dtype=torch.float32)
            self.flop += image_size**2 * conv1_out_chan * batch_size
            self.conv2 = torch.nn.Conv2d(conv1_out_chan,conv2_out_chan,conv2_kern).to(device, dtype=torch.float32)
            ...
            self.fc1   = torch.nn.Linear(conv2_out_chan * conv2_kern * conv2_kern, fc1_out).to(device, dtype=torch.float32)
            self.flop += (2*self.view_size - 1) * fc1_out * batch_size
            self.fc2   = torch.nn.Linear(fc1_out, fc2_out).to(device, dtype=torch.float32)
            self.flop += (2*fc1_out - 1) * fc2_out * batch_size
            self.fc3   = torch.nn.Linear(fc2_out, fc3_out).to(device, dtype=torch.float32)
      ...
      inputs = torch.arange(batch_size * image_size**2 * conv1_in_chan,
                            dtype=torch.float, device=device).view((batch_size,conv1_in_chan,image_size,image_size))
      net = Net(batch_size,
                image_size,
                conv1_in_chan,conv1_out_chan,conv1_kern,
                pool_size,
